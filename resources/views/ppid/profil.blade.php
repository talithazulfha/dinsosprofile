                        ...
                        @for($i = 1; $i <= $clusterData['meta']['last_page']; $i++)
                            <a href="{{ route('ppid.profil', ['page' => $i]) }}"
                               class="relative inline-flex items-center px-4 py-2 text-sm font-medium
                                    {{ request()->get('page') == $i || (!request()->has('page') && $i == 1)
    ...
    @if(config('app.debug'))
        <div class="mt-8">
            <details class="bg-gray-50 rounded-lg">
                <summary class="px-4 py-2 cursor-pointer text-sm text-gray-600 hover:text-[#0E0C43]">
                    Debug Information
                </summary>
                <pre class="p-4 bg-white text-xs overflow-x-auto border-t border-gray-100">
